...
namespace CountingBot.Modules
{
    public class GetUserCount : ModuleBase<SocketCommandContext>
    {
        ...
        public static async Task IncrementUserCountAsync(SocketGuildUser u)
        {
            string update = "UPDATE UserCounts SET count = count + 1 WHERE guild_id = @guild_id AND user_id = @user_id;";
            string insert = "INSERT INTO Channels (guild_id, user_id, count) SELECT @guild_id, @user_id, 0 WHERE (SELECT Changes() = 0);";
        ...
        {
            string delete = "DELETE FROM Channels WHERE guild_id = @guild_id AND user_id = @user_id;";
